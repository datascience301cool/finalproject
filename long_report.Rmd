...
I was curious how the model would do at predicting the price of a hypothetical used car listing on Craigslist. We found a specific listing, a 2012 Toyota RAV4, created a data frame containing the car's characteristics, and used this data frame and the random forest model to predict the `price` of the car.
...
Given that this particular listing's `price` was on low side, we  also wanted to try predicting the `price` of a more expensive listing. We found a listing for a 2016 Ford F250 listed for $30,000. Again, we  created a data frame containing the truck's characteristics and used the data frame and the random forest model to predict the `price` of the truck.
...
After undoing the log transformation, the predicted `price` ended up being $31,623, which was also pretty close to the actual `price`. We were pretty satisfied with these results, but seeing that the model underestimated the `price` in the first test and then overestimated it in the second test, we knew that the model could be improved.

As for next steps, we  would try out other models or tuning parameters to improve the model. Unfortunately, running `tune_grid()` on the random forest model took several hours, so we were not able to perform very much trial and error or try out different models like boosted trees, which are even more resource intensive than random forests. However, if given more time we  would want to try out such models.

Ultimately, we  was quite satisfied with the predictive accuracy of the random forest model given the amount of time we  had to work on the project. We believe it could be a useful tool for used car shoppers who are trying to assess the pricing of a particular listing. However, if given more time, my next steps would be to try new models and/or tuning parameters to get an even lower RMSE.
